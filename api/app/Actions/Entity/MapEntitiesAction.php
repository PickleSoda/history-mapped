...
class MapEntitiesAction
{
    ...
    public function __invoke(array $filters): array
    {
        ...
        $fallbackQuery = DB::table('entities')
            ->selectRaw(
                <<<'SQL'
                NULL::uuid AS geometry_period_id,
                entities.entity_id,
                tr.start_year,
                tr.end_year,
                'location_fallback'::text AS period_type,
                COALESCE(
                    (
                        SELECT entity_aliases.name
                        FROM entity_aliases
                ...
                entities.icon_class,
                entities.impact_score,
                ST_AsGeoJSON(COALESCE(el.territory_geom, el.geom), 5)::text AS geojson
                SQL
            )
            ->join('entity_locations as el', function ($join): void {
                $join->on('el.entity_id', '=', 'entities.entity_id')
                    ->where('el.is_primary', true);
            })
            ->leftJoin('entity_temporal_ranges as tr', function ($join): void {
                $join->on('tr.entity_id', '=', 'entities.entity_id')
                    ->where('tr.is_primary', true);
            })
            ->whereNotExists(function ($sub): void {
                $sub->selectRaw('1')
                    ->from('geometry_periods')
                    ->whereColumn('geometry_periods.entity_id', 'entities.entity_id');
            })
            ->where(function ($spatialTypeQuery): void {
                $spatialTypeQuery
                    ->whereNotNull('el.territory_geom')
                    ->orWhereNotNull('el.geom');
            })
            ->whereIn('entities.verification_status', [
                VerificationStatus::PipelineDraft->value,
                VerificationStatus::OhmDraft->value,
                VerificationStatus::AutoValidated->value,
                VerificationStatus::NeedsReview->value,
            ...
            ->where(function ($q) use ($year): void {
                $q->whereNull('tr.start_year')
                    ->orWhere('tr.start_year', '<=', $year);
            })
            ->where(function ($q) use ($year): void {
                $q->whereNull('tr.end_year')
                    ->orWhere('tr.end_year', '>=', $year);
            });

        if ($minImpact !== null) {
            $fallbackQuery->where('entities.impact_score', '>=', $minImpact);
        }

        if (isset($filters['type'])) {
            $fallbackQuery->where('entities.entity_type', EntityType::from($filters['type'])->value);
        }
        ...
        if (isset($filters['types']) && is_array($filters['types'])) {
            ...
            $fallbackQuery->whereIn('entities.entity_type', $typeValues);
        }

        if (isset($filters['min_confidence'])) {
            $fallbackQuery->where('entities.confidence', '>=', ConfidenceLevel::from($filters['min_confidence'])->value);
        }

        if (isset($filters['temporal_start'], $filters['temporal_end'])) {
            $fallbackQuery->where(function ($q) use ($filters): void {
                $q->whereNull('tr.start_year')
                    ->orWhere('tr.start_year', '<=', (int) $filters['temporal_end']);
            })->where(function ($q) use ($filters): void {
                $q->whereNull('tr.end_year')
                    ->orWhere('tr.end_year', '>=', (int) $filters['temporal_start']);
            });
        }

        $fallbackQuery->whereRaw(
            '(el.territory_geom && ST_MakeEnvelope(?, ?, ?, ?, 4326)
              OR el.geom && ST_MakeEnvelope(?, ?, ?, ?, 4326))',
            [...$bbox, ...$bbox],
        );

        $fallbackQuery
            ->orderByDesc('entities.display_priority')
            ->orderByDesc('tr.start_year')
            ->orderBy('tr.end_year');

        $fallbackFeatures = $fallbackQuery->limit($limit)->cursor()->map(function ($row): array {
            return [
                'id' => $row->entity_id,
                'geometry_json' => is_string($row->geojson) && $row->geojson !== '' ? $row->geojson : 'null',
                'properties' => [
        ...
        $features = $periodFeatures->concat($fallbackFeatures);

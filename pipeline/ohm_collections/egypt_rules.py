...
def evaluate_candidate(candidate: dict[str, Any]) -> dict[str, Any]:
    if _is_maritime_boundary(candidate):
        return {
            "include": False,
            "matched_terms": [],
            "reasons": ["maritime_boundary_excluded"],
            "score": 0,
            "ambiguity": [],
        }

    if _is_modern_egypt_state(candidate):
        return {
            "include": False,
            "matched_terms": [],
            "reasons": ["modern_egypt_state_excluded"],
            "score": 0,
            "ambiguity": [],
        }
    ...
    if isinstance(raw_tags, dict):
        for key, value in raw_tags.items():
            ...
            if normalized_key is None:
                continue
            if normalized_key == "wikipedia" or normalized_key.startswith("wikipedia:"):
                texts.append(value)
                continue
            if normalized_key == "source" or normalized_key.startswith("source:"):
                texts.append(value)
                continue
            if normalized_key in {"description", "note"}:
                texts.append(value)
                continue
            if normalized_key in {"historic", "historic:civilization", "religion"}:
                texts.append(value)
                continue
            if normalized_key in {"place", "boundary", "type", "country", "monarchy", "border_type", "empire"}:
                texts.append(value)
                continue
            if normalized_key.startswith("name:") or normalized_key.startswith("alt_name:"):
                texts.append(value)
...
def _is_maritime_boundary(candidate: dict[str, Any]) -> bool:
    raw_tags = candidate.get("raw_tags") or {}
    if not isinstance(raw_tags, dict):
        return False
    return _normalize_text(raw_tags.get("maritime")) == "yes"


def _is_modern_egypt_state(candidate: dict[str, Any]) -> bool:
    raw_tags = candidate.get("raw_tags") or {}
    if not isinstance(raw_tags, dict):
        return False

    normalized_name = _normalize_text(candidate.get("name"))
    normalized_name_en = _normalize_text(raw_tags.get("name:en"))
    normalized_name_ar = _normalize_text(raw_tags.get("name:ar"))
    modern_names = {
        "egypt",
        "republic of egypt",
        "kingdom of egypt",
    ...
    return any(
        normalized_name == modern_name or normalized_name_en == modern_name or normalized_name_ar == modern_name

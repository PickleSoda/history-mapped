...
from pipeline.wikidata.scraper.wikidata import WikidataScraper
from pipeline.wikidata.scraper.wikipedia import WikipediaEnricher
from pipeline.wikidata.scraper.topic import TopicScraper
...
from pipeline.wikidata.resolver.geo_resolver import resolve_batch
...
def topic(query, depth, limit, co_seeds, skip_wikipedia, skip_untyped, output_dir):
    ...
    Examples:
        ...
        python -m pipeline.wikidata topic "Late Bronze Age Collapse"

        python -m pipeline.wikidata topic Q484954 --depth 3 --limit 500

        python -m pipeline.wikidata topic "Roman Empire" --depth 1 --limit 100

        python -m pipeline.wikidata topic "Silk Road" --skip-wikipedia

        python -m pipeline.wikidata topic "Late Bronze Age Collapse" --co-seed Q4496560 --co-seed Q193850
    """
    out = Path(output_dir or settings.output_dir)
    out.mkdir(parents=True, exist_ok=True)

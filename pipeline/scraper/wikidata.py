...
class WikidataScraper:
    ...
    def query_entities(
        ...
        - inception: start date ISO string
        - dissolution: end date ISO string
        ...
        # Fetch additional properties (relationships, details) in batches
        if items and config.get("property_queries"):
    ...
    def _build_query(
        ...
        # Build the query
        query = f"""
        SELECT DISTINCT ?item ?itemLabel ?itemDescription ?coord ?inception ?dissolution
               ?wikipedia_title
        WHERE {{
    ...
    def _enrich_properties(
        ...
        property_ids = config.get("property_queries", [])
        if not property_ids:
            return items

        # Process in chunks of 50 to stay within SPARQL limits
        chunk_size = 50

...
@pytest.mark.parametrize(
    ...
    [
        ({"name": "Ancient Egypt", "entity_types": ["historical_period"]}, "egypt"),
...
def test_evaluate_candidate_excludes_modern_egypt_state_forms(candidate_name: str) -> None:
    decision = evaluate_candidate({"name": candidate_name, "entity_types": ["political_entity"]})

    assert decision["include"] is False
    assert "modern_egypt_state_excluded" in decision["reasons"]
...
def test_evaluate_candidate_excludes_incidental_egypt_mentions_in_event_metadata() -> None:
    decision = evaluate_candidate(
        {
            "name": "British Empire",
            "raw_tags": {"end_event": "Egypt independence"},
...
def test_evaluate_candidate_excludes_maritime_boundaries_even_when_they_name_egypt() -> None:
    decision = evaluate_candidate(
        {
            "name": "3nm line - Egypt",
            "raw_tags": {"maritime": "yes"},
            "entity_types": ["political_entity"],
        }
    )

    assert decision["include"] is False
    assert "maritime_boundary_excluded" in decision["reasons"]
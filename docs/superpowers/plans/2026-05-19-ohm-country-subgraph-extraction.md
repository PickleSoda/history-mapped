...
- [ ] **Step 1: Write a fixture-sized graph test for bidirectional expansion**
...
- [ ] **Step 2: Write closure-report contract tests**
...
- [ ] **Step 1: Implement seed resolution helpers**
Support seed lookup by explicit QID and exact-name fallback within the provided Overpass payload.

- [ ] **Step 2: Implement adjacency indexing and recursive traversal**
...
- [ ] **Step 3: Preserve resume/force semantics**
Ensure a second run with `--resume` reuses subset artifacts and `--force` rebuilds them.

- [ ] **Step 4: Persist and compare traversal parameters in the manifest**
...
- [ ] **Step 1: Add the `extract-subgraph` command to the OHM borders CLI**
Expose `--input`, `--seed-qid`, `--seed-name`, `--run-id`, `--artifact-dir`, `--max-depth`, `--max-nodes`, `--raw-shard-size`, `--resume`, and `--force`.

- [ ] **Step 2: Expose the same command via the top-level legacy dispatcher**
...
- [ ] **Step 3: Implement import-readiness validation**
...
- [ ] **Step 4: Add tests for import-readiness validation**
...
- [ ] **Step 5: Run focused downstream tests**
...
- [ ] **Step 1: Write a first-run guide**
...
- [ ] **Step 2: Add second-run guidance**
Document `--resume`, `--force`, and how to reuse an existing subset artifact directory safely.

- [ ] **Step 3: Add validation, seed-failure, and import-order guidance**
...
- [ ] **Step 4: Update the OHM borders README**
...
- [ ] **Step 1: Run the new extractor test file**
Run: `py -m pytest pipeline/tests/test_ohm_country_subgraph_extractor.py -v`
Expected: PASS.

- [ ] **Step 2: Run existing OHM relation tests affected by the new workflow**
...
- [ ] **Step 3: Run existing OHM stage tests that cover artifact flow**
Run: `py -m pytest pipeline/tests/test_ohm_borders_stages.py -v`
Expected: PASS.

- [ ] **Step 4: Perform a manual CLI smoke test against a small fixture or subset payload**
